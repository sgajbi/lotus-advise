...
def test_quality_baseline_report_captures_required_quality_sections(tmp_path: Path) -> None:
    ...
    assert "enrichment, tactical house-view, and policy workflow projection boundaries" in (
        refactor_health
    )
    ...
    assert "Review ledger includes `LA-REV-611` through `LA-REV-866`" in scorecard
