...
The app-local Compose manifest supplies `LOTUS_ADVISE_TENANT_ID=tenant-sg-001` only as a standalone
developer fallback. The governed Workbench canonical launcher injects the source tenant from the
platform demo-data contract, and targeted refreshes must preserve that injected value. Production
Compose still requires deployment-owned tenant configuration and must not use either local value as
tenant-isolation or entitlement proof.
