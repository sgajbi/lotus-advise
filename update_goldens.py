...
def save_golden(name: str, inputs: dict, result):
    ...
    # Convert Pydantic model to dict, handling Decimals via json_encoders if needed,
    # but model_dump_json() is safer, then load back to dict for clean JSON structure.
    output_json = json.loads(result.model_dump_json())
    
    # We strip dynamic IDs to ensure deterministic comparison in tests
    ...
    data = {
        "scenario_name": name,
        "inputs": json.loads(json.dumps(inputs, default=str)),  # Simple Decimal serializer
...
def generate_scenarios():
    ...
    md_01 = MarketDataSnapshot(prices=[Price(instrument_id="EQ_1", price=Decimal("100.0"), currency="SGD")])
    opts_01 = EngineOptions()
    
    res_01 = run_simulation(base_pf, md_01, model_01, shelf_01, opts_01)
    save_golden("scenario_01_cash_inflow", {
    ...
    model_02 = ModelPortfolio(targets=[
    ...
    save_golden("scenario_03_fx_funding", {
    ...
    save_golden("scenario_04_dq_failure", {
        "portfolio_snapshot": base_pf.model_dump(),
    ...
    md_05 = MarketDataSnapshot(prices=[Price(instrument_id="EQ_1", price=Decimal("100.0"), currency="SGD")])
    ...
    save_golden("scenario_05_constraint_fail", {
        "portfolio_snapshot": base_pf.model_dump(),
    ...
    shelf_06 = [ShelfEntry(instrument_id="EQ_1", status="APPROVED", min_notional=Money(amount=Decimal("50000.0"), currency="SGD"))]
    md_06 = MarketDataSnapshot(prices=[Price(instrument_id="EQ_1", price=Decimal("100.0"), currency="SGD")])
    opts_06 = EngineOptions(suppress_dust_trades=True)

    res_06 = run_simulation(base_pf, md_06, model_06, shelf_06, opts_06)
    save_golden("scenario_06_suppression", {
        "portfolio_snapshot": base_pf.model_dump(),
    ...
    save_golden("scenario_07_audit_trace", {
        "portfolio_snapshot": base_pf.model_dump(),
        "market_data_snapshot": md_02.model_dump(),
        "model_portfolio": model_02.model_dump(),
        "shelf_entries": [s.model_dump() for s in shelf_02],
