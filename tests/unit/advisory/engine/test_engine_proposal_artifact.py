...
def test_proposal_artifact_builder_delegates_projection_helpers():
    ...
    assert "from src.core.advisory.artifact_models import (\n" not in (
        REPO_ROOT / "src/core/advisory/artifact_portfolio.py"
    ).read_text(encoding="utf-8")
    assert "from src.core.advisory.artifact_models import ProposalArtifactTakeaway" not in (
        REPO_ROOT / "src/core/advisory/artifact_summary.py"
    ).read_text(encoding="utf-8")
    assert "from src.core.advisory.artifact_models import (\n" not in (
        REPO_ROOT / "src/core/advisory/artifact_trades.py"
    ).read_text(encoding="utf-8")
    assert "from src.core.advisory.artifact_models import (\n" not in (
        REPO_ROOT / "src/core/advisory/artifact_review.py"
    ).read_text(encoding="utf-8")
