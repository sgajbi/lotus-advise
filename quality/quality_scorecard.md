...
| --- | --- | --- | --- |
| Complexity | Radon and Xenon tracked as pending report-only tools. | Radon config executable; inventory `A=3059, B=253, C=7`; worst block `C/14`; no E/F gate enforced through `make lint`. | Complexity is now measured repeatably and regression-blocked for E/F-ranked blocks. |
| Maintainability | Review ledger existed but recent proposal, policy-pack, OpenAPI, proof-material, dependency-linking, and observability slices were absent. | Review ledger includes `LA-REV-611` through `LA-REV-739` with scoped findings, evidence, and follow-up. | Modularization and hotspot reductions are traceable by owner boundary and test evidence. |
...
| Tests | Unit suite existed; new focused refactor regressions were absent. | Unit suite now has `1941` passing tests in `make check`; added focused tests for OpenAPI enrichment, proof refs, source refs, dependency linking, structured logging, and advisor cockpit action aggregation. | Refactors are covered by behavior-preserving regression tests. |
