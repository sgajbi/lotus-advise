...
- Advisory workspace routes now compose through a workspace application service with explicit
  repository, source-context, proposal-evaluator, and proposal-lifecycle ports while keeping the
  current in-memory session store as an infrastructure adapter.
