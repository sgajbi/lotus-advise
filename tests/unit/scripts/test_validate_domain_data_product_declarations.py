...
def test_rfc0027_copilot_products_are_not_promoted_before_runtime_proof() -> None:
    contract_dir = _local_contract_dir(Path(__file__).resolve().parents[3])
    declaration = json.loads(
        (contract_dir / "lotus-advise-products.v1.json").read_text(encoding="utf-8")
    )
    ...
    assert "AdvisoryCopilotInteractionRecord" not in products
    assert "AdvisoryCopilotEvidencePacket" not in products
    assert "AdvisoryCopilotReviewRecord" not in products
    routes = [route for product in products.values() for route in product["current_routes"]]
    assert not any("copilot" in route.lower() for route in routes)
