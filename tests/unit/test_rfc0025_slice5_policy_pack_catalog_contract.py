...
def test_rfc0025_slice5_promotes_catalog_without_policy_evaluation_claims() -> None:
    ...
    assert "POLICY_PACK_MAKER_CHECKER_REQUIRES_DIFFERENT_ACTOR" in CATALOG_SOURCE_PATH.read_text(
        encoding="utf-8"
    )
    assert "POLICY_PACK_VERSION_ALREADY_ACTIVE_IMMUTABLE" in CATALOG_SOURCE_PATH.read_text(
        encoding="utf-8"
    )
