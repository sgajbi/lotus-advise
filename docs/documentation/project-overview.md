...
- Workspace sessions and saved workspace versions are owned by the `workspace` Postgres migration
  namespace and Postgres repository adapter. Production runtime validation requires
  `WORKSPACE_STORE_BACKEND=POSTGRES`; the in-memory workspace adapter is retained only for
  non-release/test use.
