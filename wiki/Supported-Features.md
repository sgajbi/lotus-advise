...
| `RFC-0026` | Advisor cockpit operating workflow | Creates backend-owned advisor worklists, action items, meeting-preparation packets, and workflow readiness summaries. | Ready for implementation after 2026-05-27 gold-standard tightening. Slice 1 is complete as platform-scaffolding review only; no `lotus-platform` code change is required before cockpit domain work. Slice 2 adds the `src/core/advisor_cockpit/` domain package, typed models, vocabulary, sorting, and pagination defaults. Slice 3 records non-promoting data-product posture and blocks premature cockpit declarations, trust telemetry, and `/platform/capabilities` promotion. Slice 4 adds pure Advise core source-backed action construction for policy review, memo blockers, meeting preparation, source supportability, unsupported capabilities, and every first-wave action family through the generic builder. Slice 5 adds preloaded source-read-model aggregation for proposals, policy evaluations, memos, supportability, and unsupported-capability markers. Slice 6 adds deterministic SLA age-band and acknowledgement posture rules that cannot clear blocking ownership. Slice 7 adds Advise-owned cockpit action, snapshot, supportability, and idempotent acknowledgement APIs with durable acknowledgement persistence. No integrated advisor-cockpit product support claim is promoted until Gateway, Workbench, data-product, canonical `RFC26_ADVISOR_COCKPIT_CANONICAL` automation, docs, and live evidence are complete. |
