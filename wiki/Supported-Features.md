...
| `RFC-0024` | Advisor proposal memo and evidence pack | Turns proposal evidence into an advisor, compliance, operations, audit, and sales-ready memo package. | Slices 0-4 are complete as non-claiming source-map, product-gap allocation, platform-scaffolding, cleanup/structure, proposed/blocked data-product posture, and persisted source-readiness evidence. Slices 0-5 are complete when the Slice 5 deterministic pure memo-builder foundation is included. Slices 0-6 are complete when the Slice 6 durable memo persistence, idempotency, replay metadata, and audit-event foundation is included. Slice 7 is complete for canonical `lotus-advise` memo create/read/projection/review/report-package-event/lineage/replay APIs and certified OpenAPI. Slice 8 is complete for memo-critical suitability and product eligibility evidence, cost/fee/tax/friction limitation evidence, risk disclosure and product-document evidence, and conflict blocker enrichment without positive client-ready wording. Slice 9 is complete for advisor-use report/render/archive realization: Advise requires exact memo hash continuity and `APPROVE_FOR_ADVISOR_USE` review before requesting a typed memo package from `lotus-report`; Report preserves the package in the immutable snapshot and deterministic render package; Archive stores support-safe memo archive metadata; and Advise memo lineage records report/render/archive refs. Slice 10 is complete for review-gated advisor-use AI commentary through `proposal_memo_commentary.pack@v1`: Advise sends bounded memo evidence only, records append-only AI lineage, returns deterministic unavailable posture when AI is not configured, and prevents AI from changing memo evidence, suitability, approval, or client-ready posture. Existing platform/repo-native controls are sufficient before memo domain work, one-off local scaffolding is rejected, reviewed narrative report-package business rules now live in the core proposal report-handoff boundary, `AdvisoryProposalMemoEvidencePack:v1` remains unpromoted and stays in proposed/blocked governance posture, proposal evidence bundles now carry `rfc0024.memo-source-readiness.v1`, and the Slice 5 pure builder projects stored proposal evidence plus that manifest into typed memo sections with explicit `PENDING_REVIEW`, `BLOCKED`, or `NOT_AVAILABLE` posture where source facts are missing. Gateway, Workbench, report/render/archive realization is no longer a single planned bundle: Slice 9 has implemented report/render/archive realization, while Gateway, Workbench, active data-product support, commercial/demo claims, and client-ready memo claims remain planned until implementation-backed slices close |
