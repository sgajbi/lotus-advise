...
| `RFC-0024` | Advisor proposal memo and evidence pack | Turns proposal evidence into an advisor, compliance, operations, audit, and sales-ready memo package. | Slices 0-3 are complete as non-claiming source-map, product-gap allocation, platform-scaffolding, cleanup/structure, and proposed/blocked data-product gates. Existing platform/repo-native controls are sufficient before memo domain work, one-off local scaffolding is rejected, reviewed narrative report-package business rules now live in the core proposal report-handoff boundary, and `AdvisoryProposalMemoEvidencePack:v1` exists only as proposed/blocked governance posture. Memo generation, memo APIs, memo persistence, report package, Gateway, Workbench, active data-product support, and client-ready memo claims remain planned until implementation-backed slices close |
