...
| API governance | Enforced for OpenAPI quality, no-alias behavior, API vocabulary, domain data products, architecture boundaries, complexity regression, high-severity security findings, and license/IP release evidence. | `make check`, `make license-ip-gate`, Feature Lane, PR Merge Gate, Main Releasability. |
