...
PATTERNS = {
    "field_alias": re.compile(r"\bField\([^)]*\balias\s*=\s*\""),
    "query_alias": re.compile(r"\bQuery\([^)]*\balias\s*=\s*\""),
    ...
    "validation_alias": re.compile(r"\bvalidation_alias\b"),
    "serialization_alias": re.compile(r"\bserialization_alias\b"),
...
def main() -> int:
    ...
    if findings:
        print("No-alias contract guard failed. Remove alias/legacy API patterns:")
