...
| Advisory copilot | Supported for governed evidence packets, approved `lotus-ai` provider/model inventory enforcement, executable evaluation-pack and safety-abuse gates, claim-level source grounding, structured guardrails, workflow-pack integration, trusted-principal run review, maker-checker audit, retention, certified APIs, and product-realization proof. Unknown, retired, mismatched, or environment-incompatible model identity returns unavailable; failed evaluation, unsafe source evidence, unsafe output, unsupported, or unverifiable AI claims remain not review-ready, and request-body actor fields cannot authorize review actions. | RFC-0027 tests, model-governance/evaluation/safety tests, and API contract evidence. |
