...
def test_golden_scenario(filename, scenario):
    ...
    assert len(result.intents) == len(expected["intents"]), (
        f"Expected {len(expected['intents'])} intents, got {len(result.intents)}"
    )
    ...
    for act_intent, exp_intent in zip(result.intents, expected["intents"]):
        ...
        if act_intent.intent_type == "SECURITY":
            ...
            assert float(act_intent.est_notional.amount) == float(
                exp_intent["est_notional"]["amount"]
            )
            ...
            assert float(act_intent.sell_amount.amount) == float(
                exp_intent["sell_amount"]["amount"]
            )
