...
def _create_policy_evaluation(*, material_conflict: bool = False):
    ...
    shelf_entry = evidence["inputs"]["shelf_entries"][0]
    shelf_entry.update(
        instrument_id="SG_STRUCTURED_NOTE", complexity="COMPLEX", structured_product=True
    )
    evidence["inputs"]["proposed_trades"][0]["instrument_id"] = "SG_STRUCTURED_NOTE"
    evidence["artifact"]["disclosures"]["product_docs"][0].update(
...
def test_policy_workflow_projects_open_approval_disclosure_consent_and_sla_posture() -> None:
    ...
    workflow = get_policy_evaluation_workflow(
        evaluation_id=record.evaluation_id, tenant_id="tenant_sg_001"
    )
