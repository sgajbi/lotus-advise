...
| Advisory workspace | Supported for draft, save, resume, compare, rationale, and lifecycle handoff flows. | Workspace API and engine tests. |
...
| Advisory copilot | Supported for governed evidence packets, guardrails, workflow-pack integration, run review, audit, retention, certified APIs, and product-realization proof. | RFC-0027 tests and API contract evidence. |
...
| API governance | Enforced for OpenAPI quality, no-alias behavior, API vocabulary, domain data products, architecture boundaries, complexity regression, and high-severity security findings. | `make check`, Feature Lane, PR Merge Gate, Main Releasability. |
