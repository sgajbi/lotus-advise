...
- `APP_PERSISTENCE_PROFILE=LOCAL`:
  ...
  - Legacy runtime backends are deprecated and retained only where explicitly required by active advisory code.
...
3. Production-profile startup smoke:
   - starts API with `APP_PERSISTENCE_PROFILE=PRODUCTION` and advisory Postgres backend.
