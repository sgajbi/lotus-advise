...
| `RFC-0024` | Advisor proposal memo and evidence pack | Turns proposal evidence into an advisor, compliance, operations, audit, and sales-ready memo package. | Slices 0-4 are complete as non-claiming source-map, product-gap allocation, platform-scaffolding, cleanup/structure, proposed/blocked data-product posture, and persisted source-readiness evidence. Existing platform/repo-native controls are sufficient before memo domain work, one-off local scaffolding is rejected, reviewed narrative report-package business rules now live in the core proposal report-handoff boundary, `AdvisoryProposalMemoEvidencePack:v1` exists only as proposed/blocked governance posture, and proposal evidence bundles now carry `rfc0024.memo-source-readiness.v1` so missing `lotus-core`, `lotus-risk`, and `lotus-advise` source facts remain explicit `PENDING_REVIEW`, `BLOCKED`, or `NOT_AVAILABLE` posture. Memo generation, memo APIs, memo persistence, report package, Gateway, Workbench, active data-product support, and client-ready memo claims remain planned until implementation-backed slices close |
