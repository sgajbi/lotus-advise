...
| `RFC-0025` | Enterprise suitability and best-interest policy packs | Adds versioned policy packs for suitability, best-interest, product eligibility, disclosures, approvals, sign-off evidence, review queues, and source-readiness gaps. | Planned RFC only. Slice 0 is tightened as the pre-implementation decision gate: first supported proof targets `GLOBAL_PRIVATE_BANKING_BASELINE` and `SG_PRIVATE_BANKING_REFERENCE`; product-governance facts remain `lotus-core` source truth; risk facts remain `lotus-risk` source truth; Workbench must consume Gateway/BFF-only policy posture; client-ready publication and external communication remain gated until policy, memo/report/render/archive, review, disclosure, consent, and RFC-0028 claim controls are implementation-backed. Slice 1 is complete as platform-scaffolding review only: existing platform/repo-native controls are sufficient, no `lotus-platform` code change is required yet, and no policy-pack runtime capability is promoted. Slice 2 is complete as current-boundary cleanup only: advisory policy-context status interpretation is centralized, duplicate suitability scanner baseline-pack wiring is removed, and dedicated policy-pack modules remain future implementation work until backed by real behavior and tests. Slice 3 is complete as proposed, blocked data-product posture only: `AdvisoryPolicyEvaluationRecord:v1` has a producer declaration and blocked trust telemetry, but policy evaluation support is Planned and is not advertised through `/platform/capabilities`. Slice 4 is complete as source-readiness evidence: proposal evidence bundles carry `rfc0025.policy-source-readiness.v1`, which separates `lotus-core`, `lotus-risk`, and `lotus-advise` source gaps as `PENDING_REVIEW` or `BLOCKED`. Slice 5 is complete as catalog and activation support only: Advise exposes policy-pack list/detail/validate/activate routes, `rfc0025.policy-pack-catalog.v1`, reference packs clearly marked `REFERENCE_EXAMPLE_NOT_LEGAL_ADVICE`, content hashes, maker-checker activation, and audit events. Slice 6 is complete as an internal policy applicability and rule-evaluation engine only: active policy packs can produce source-backed material rule posture for source readiness, mandate, product eligibility, complex-product disclosure/consent, best-interest cost evidence, and conflict/product-document review. Slice 7 is complete as internal policy evaluation persistence, replay, idempotency, and audit only: Advise persists finalized policy evaluation records with policy/source/evaluation hashes, per-rule hashes, source gaps, approval dependencies, disclosure requirements, consent requirements, replay metadata, duplicate prevention, and append-only review/sign-off/report-archive events. Slice 8 is complete for certified Advise evaluation APIs, review queue projection, replay, lineage, append-only review/sign-off/report-reference events, and sign-off source packages. Slice 9 is complete for Advise source workflow projection and sign-off decision recording over finalized policy records: approval dependencies, disclosure and consent requirements, conflict posture, SLA aging, maker-checker enforcement, source-hash validation, and append-only sign-off events are backed by the policy record and event store. Slice 10 is complete for Advise-owned policy report-package realization: signed-off policy evaluations can be submitted as typed policy sign-off packages to `lotus-report`, returned report/render/archive refs are recorded in policy lineage, idempotent replay is enforced, and client-ready document requests fail closed. Slice 11 is complete for Advise-owned AI policy-evidence consumption: finalized policy evaluations can be submitted as redacted bounded evidence to `policy_evidence_summary.pack@v1`, forbidden actions are rejected, prompt/output lineage is recorded in policy lineage, AI unavailable posture is deterministic, human review is required, AI output is non-authoritative for policy status, and client-ready publication remains blocked. Slice 12 is complete for Gateway and Workbench product realization: Gateway routes the canonical Advise policy-pack/evaluation BFF surface and Workbench consumes Gateway-only policy review posture, selected evaluation evidence, sign-off package posture, workflow posture, and bounded request-more-evidence actions without local policy calculation, approval/waiver authority, completed sign-off authority, or client-ready publication. Slice 13 is complete for policy-pack-specific commercial support material in `docs/commercial/RFC-0025-enterprise-policy-pack-commercial-support.md`, including claim-controlled one-pager language, demo notes, API examples, architecture flow, operator guidance, security posture, and RFP-safe wording. Slice 14 is complete for policy evaluation implementation proof in the live runtime evidence bundle: `proposal_policy` covers Advise policy evaluation APIs, SG reference-pack hashes, source refs and gaps, requirement counts, workflow and sign-off posture, report/render/archive refs or degraded reason, bounded AI evidence, lineage, replay hashes, and stale-hash/client-ready/forbidden-AI blocked paths. active data-product promotion, final hardening, final closure, and post-completion communication remain gated. |
...
Historical RFC-0025 Slice 7-13 gate language remains intentionally preserved for auditability:
Canonical live proof, active data-product promotion, final hardening, final closure, and
post-completion communication were the remaining gates before Slice 14 added live-suite
