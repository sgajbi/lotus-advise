...
| `RFC-0024` | Advisor proposal memo and evidence pack | Turns proposal evidence into an advisor, compliance, operations, audit, and sales-ready memo package. | Slices 0-2 are complete as non-claiming source-map, product-gap allocation, platform-scaffolding, and cleanup/structure gates. Existing platform/repo-native controls are sufficient before memo domain work, one-off local scaffolding is rejected, and reviewed narrative report-package business rules now live in the core proposal report-handoff boundary. Memo generation, memo APIs, memo persistence, report package, Gateway, Workbench, data-product promotion, and client-ready memo claims remain planned until implementation-backed slices close |
