...
| `RFC-0024` | Advisor proposal memo and evidence pack | Turns proposal evidence into an advisor, compliance, operations, audit, and sales-ready memo package. | Slices 0-1 are complete as non-claiming source-map, product-gap allocation, and platform-scaffolding gates. Existing platform/repo-native controls are sufficient before memo domain work, and one-off local scaffolding is rejected. Memo generation, memo APIs, memo persistence, report package, Gateway, Workbench, data-product promotion, and client-ready memo claims remain planned until implementation-backed slices close |
